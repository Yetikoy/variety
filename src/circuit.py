...
class Circuit:
    ...
    def attach(self, element_factory, node1, node2, *args, **kwargs):
        ...
        # we explicitly do it through function call to capture transient value instead of binding to loop variable
        def adapt_lens(lens): return lambda xs, state, err: lens(xs[index2]-xs[index1], state[element.name])
        for lens_name, lens in element.lenses:
            self.lenses.append([f'{lens_name} at {element.name}', adapt_lens(lens)])
    def compile_current_funcs(self):
        ...
        def get_snode_current_calculators():
            ...
            def mk_updater(edge, is_inflow=True):
                ...
                def updater(edge_currents, node_currents):
                    #print('updating for', edge, reference_node, is_inflow)
                    current = node_currents[reference_node] * (-1 if is_inflow else 1)
            ...
            for element_name in self.elements:
                if element_name not in calculated_edges:
                    ...
                    # TODO: this will not work due to binding to loop variable
        ...
        def get_currents_simple(voltages, state):
            ...
            #print('currents simple, edges: ', edge_currents)
            #print('currents simple, nodes: ', node_currents)
        ...
        def get_currents_full(voltages, state):
            ...
            #print('edge currents', edge_currents)
            #print('node currents', node_currents)
            return edge_currents, node_currents
